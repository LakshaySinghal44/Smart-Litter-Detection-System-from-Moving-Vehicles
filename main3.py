...
def send_gmail_threaded(
    ...
    # Use threading to send the email without blocking the main thread
    threading.Thread(
        target=send_gmail,
...
def main(video_path):
    ...
    litter_frame_counter = (
        0  # Counter to keep track of the frames to display "Litter Detected"
    )
    litterCount = 0
    ...
    while True:
        ...
        if litter_frame_counter > 0:
            litter_frame_counter -= 1  # Decrement counter if it's greater than 0
        else:
            cv2.putText(
                result_frame,
                "Normal",
